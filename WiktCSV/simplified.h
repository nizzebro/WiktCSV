...
class basic_charsor {
    ...
    constexpr bool empty() const noexcept {return _end == _ptr;}
    ...
    constexpr U peekc() noexcept { return empty()? **this : 0;}
    ...
    static constexpr auto is_eq(U ch) noexcept 
    { return [&](U c){return c == ch;};}
    static constexpr auto is_lt(U ch) noexcept 
    { return [&](U c){return c < ch;};}
    static constexpr auto is_gt(U ch) noexcept 
    { return [&](U c){return c > ch;};}   
    static constexpr auto is_lt_eq(U ch) noexcept 
    { return [&](U c){return c <= ch;};} 
    static constexpr auto is_gt_eq(U ch) noexcept
    { return [&](U c){return c >= ch;};}  
    static constexpr auto is_space() noexcept 
    { return [](U c){return std::isspace(c);};}
    static constexpr auto is_punct() noexcept 
    { return [](U c){return std::ispunct(c);};}
    static constexpr auto is_blank() noexcept
    { return [](U c){return std::isblank(c);};}
    static constexpr auto is_alnum() noexcept
    { return [](U c){return std::isalnum(c);};}
    static constexpr auto is_print() noexcept 
    { return [](U c){return std::isprint(c);};}
    static constexpr auto is_graph() noexcept
    { return [](U c){return std::isgraph(c);};}
    static constexpr auto is_cntrl() noexcept 
    { return [](U c){return std::iscntrl(c);};}
    static constexpr auto is_digit() noexcept 
    { return [](U c){return std::isdigit(c);};}

    template <typename P>
    static constexpr auto is_not(P p) noexcept 
    { return [&](U c){return !p(c);};}

    static constexpr auto is_of(std::initializer_list<T> l) noexcept 
    {
        return [&](U c) {for (auto ch : l) { if (ch == c) return true; }  return false; };
    }
    ...
    constexpr static auto char_handler_stub = [](U c) {};
    ...
    { 
        ...
        if(n) 
        {
            while (!q(*p)) { h(*p);  _add<R>(p, 1); if (--n == 0) break; }
            _ptr = p;
        }
        return  n? *p : 0;
    }
    ...
    constexpr U seek(U ch, H handler = char_handler_stub) noexcept 
    {  return seek_if(is_eq(ch), handler); }

    template<typename H = decltype(char_handler_stub)>
    constexpr U seek_of(const T* cstr, H handler = char_handler_stub) noexcept
    {   
        auto n = size();
        if(n) 
        {
    ...
    template<typename H= decltype(char_handler_stub)>
    constexpr std::size_t skipstr(const T* substr, size_t n, H handler = char_handler_stub) noexcept  
    {
        n = std::min(size(), n);
        if(n)
        {
    ...
    template<typename H= decltype(char_handler_stub)>
    constexpr std::size_t skipstr(const T* substr, H handler = [](U c){}) noexcept  
    {
        return skipstr(substr, std::char_traits<T>::length(substr), handler);
    } 
    template<typename H= decltype(char_handler_stub)>
    constexpr std::size_t skipstr(const stl_string_view& substr, H handler = [](U c){}) noexcept  
    {
        return skipstr(substr.data(), substr.size(), handler);
    }

    template<typename A, typename H= decltype(char_handler_stub)> 
    constexpr std::size_t skipstr(const stl_string<A>& substr, H handler = [](U c){}) noexcept  
    {
        return skipstr(substr.data(), substr.size(), handler);
    }
    ...
    */
    template<typename H= decltype(char_handler_stub)>
    constexpr bool seekstr(const T* cstr, size_t n, H handler = char_handler_stub) noexcept 
    { 
        auto nc; auto b;
        for(;;)
        {   
    ...
    template<typename H= decltype(char_handler_stub)>
    constexpr bool seekstr(const T* s, H handler = char_handler_stub) noexcept
    {
        return seekstr(s, std::char_traits<T>::length(s), handler); // need to know str len in advance
    }
    template<typename H= decltype(char_handler_stub)>
    constexpr bool seekstr(const stl_string_view& s, H handler = char_handler_stub) noexcept  
    {
        return seekstr(s.data(), s.size(), handler);
    }
    template<typename A, typename H= decltype(char_handler_stub)> 
    constexpr bool seekstr(const stl_string<A>& s, H handler = char_handler_stub) noexcept  
    {
        return seekstr(s.data(), s.size(), handler);
    }
    ...
    constexpr U seek_append_if(Predicate predicate, stl_string<A>& dst) noexcept  
    {
        return  seek_if(predicate, [&] (U c) {dst += c;});
    }
